...
| **特殊機能** | 旋空（射程×2の斬撃・6m）／Speed I（移動速度上昇） | | | | |
...
|---|---|---|---|---|---|
| 6 | 2m | 5 | 片手 | 8 tick | 4 |
| **特殊機能** | 全方位斬撃（360°範囲攻撃）／Slow Falling（落下ダメージ無効） | | | | |
...
| **特殊機能** | 剣⇔盾モード切替（Shift＋右クリック）、盾モード時35%軽減／Slowness I | | | | |
...
| **特殊機能** | 3連射（威力×0.7、弾速×1.2）／片手運用可（弧月と同時装備可） | | | | |
...
| **特殊機能** | 5連射（威力×0.6、弾速×1.3、微拡散）／弾薬スロット×2対応 | | | | |
...
| **特殊機能** | 集中射撃（拡散を絞り威力×1.5）／5m以内で近距離ボーナス×1.5 | | | | |
...
| **特殊機能** | 8連射（威力×0.5、弾速×1.5、拡散あり）／Slowness II（重火器） | | | | |
...
| **特殊機能** | 高速爆発弾（弾速×1.5、威力×1.2、爆発×1.5）／着弾にSlowness付与 | | | | |
...
| **特殊機能** | 最大6発を事前設置し一斉発射／設置弾は30秒で自動消滅・CRITパーティクルで可視化 | | | | |
...
| **特殊機能** | 高速爆発弾（弾速×2、爆発×1.5）／範囲3mのスプラッシュダメージ | | | | |
...
| **特殊機能** | 追尾弾3連射（上下±10°扇状展開）／Glowing（20m以内の敵を透視） | | | | |
...
| **特殊機能** | 縦方向3発同時発射（それぞれ独立して操舵可）／極低速弾を自在に曲げられる | | | | |
...
| **特殊機能** | 範囲スロウネス弾（3m内の全敵にSlowness III）／シールド貫通 | | | | |
...
| **特殊機能** | チャージショット（威力×2.5、弾速×3）／トリオン量が多いほど射程↑ | | | | |
...
| **特殊機能** | 連続高速弾5発同時発射（弾速×4）／トリオン量が多いほど弾速↑ | | | | |
...
| **特殊機能** | 爆破弾（威力×2＋着弾爆発・ブロック破壊）／Slowness I／トリオン量が多いほど威力↑ | | | | |
...
|---|---|---|---|---|
| 50%（集中75%） | 3 | 片手 | — | 3 |
| **特殊機能** | 集中（軽減率75%に引き上げ、持続1.5秒に短縮）／レッドバレットは貫通 | | | | |
...
|---|---|---|---|
| 10 | 両手 | — | 12 |
| **特殊機能** | カタパルト（真下に設置→上方向へ強制ジャンプ）／設置後は取り消し不可 | | |
...
|---|---|---|
| 2 / 秒（継続） | — | 3 |
| **特殊機能** | レーダー非表示 ON/OFF切り替え／装備中は毎秒2トリオンを継続消費 | | |
...
|---|---|---|
| 4 | 10 tick | 4 |
| **特殊機能** | 空中足場設置（5秒持続、同時最大4枚）／上限超えると最古から消滅 | | |
...
|---|---|---|
| 4 | — | 4 |
| **特殊機能** | ワイヤー弾（単体Slowness）／ワイヤー網展開（周囲全エンティティにSlowness） | | |
...
|---|---|---|
| 3 | — | 5 |
| **特殊機能** | 一斉起爆（周囲全エンティティにダメージ＋ノックバック）／同時設置上限3個 | | |
...
| **特殊機能** | ラジアルメニューで4モード切替（Shift長押し）／バイパー・アステロイド・ハウンド・メテオラの合成弾 | | | | |
